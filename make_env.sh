...
cd packages/main/bilby_main && pip install -r requirements.txt && python setup.py develop && cd -
deactivate
...
cd  packages/modded/bilby_modded && pip install -r requirements.txt && git checkout -b refactor_rwalk_to_return_more_points origin/refactor_rwalk_to_return_more_points && python setup.py develop && cd -
deactivate 
