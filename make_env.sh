...
cd packages/main/bilby_main && pip3 install -r requirements.txt && python setup.py develop && pip3 install bilby_pipe && cd -
deactivate
...
cd  packages/modded/bilby_modded && pip3 install -r requirements.txt && git checkout -b refactor_rwalk_to_return_more_points origin/refactor_rwalk_to_return_more_points && python setup.py develop && pip3 install bilby_pipe && cd -
deactivate 
